...
public class ApiClientGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var interfaceDeclarations = context
            .SyntaxProvider.CreateSyntaxProvider(
                predicate: (s, _) =>
                    s is InterfaceDeclarationSyntax { Identifier: { Text: var text } }
                    && text.EndsWith("Api"),
        ...
        context.RegisterSourceOutput(
            ...
            (spc, interfaceDecls) =>
            {
                foreach (var interfaceDecl in interfaceDecls)
                {
                    ...
                    var fileName = $"{interfaceDecl.Identifier.Text}Client.g.cs";
    ...
    )
    {
        ...
        source.AppendLine(
            $$"""        var methodCallInfo = _endpointInvoker.GetMethodCallInfo<{{interfaceIdentifier}}>("{{methodName}}");"""
        );
        source.AppendLine();
        ...
        if (pathParams.Any())
        {
            ...
            {
                source.AppendLine(
                    $"            {{ \"{param.Identifier}\", {param.Identifier}.ToString() }},"
                );
            }
            source.AppendLine("        };");
            source.AppendLine();
        ...
        source.AppendLine(
            $"            {(bodyParam != null ? bodyParam.Identifier.ToString() : "null")}"
        );
        source.AppendLine("        );");
        source.AppendLine("    }");
    }
